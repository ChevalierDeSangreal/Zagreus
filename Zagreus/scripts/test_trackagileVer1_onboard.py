...
from Zagreus.utils import velh_lossVer5, agile_lossVer1, AgileLoss, agile_lossVer6, euler2qua
...
def get_args():
	custom_parameters = [
		...
		{"name": "--experiment_name", "type": str, "default": "test_trackagileVer1_onboard", "help": "Name of the experiment to run or load."},
...
async def run():
	...
	with torch.no_grad():
		...
		for step in range(args.len_sample):
			...
			tmp_input = torch.cat((now_quad_state[:, 6:9], now_quad_state[:, 9:], now_quad_state[:, 3:6], body_rel_dis), dim=1)
			...
			input_action[:, 0] = - action[:, 0] * (2 * mass * 9.81) + mass * 9.81
			input_action[:, 1:] = action[:, 1:] * 180 / np.pi * 3
			...
			loss_agile, new_loss = agile_lossVer6(old_loss, now_quad_state, tar_state, 7, tar_ori, 2, timer, dt, init_vec, action, last_action)
			old_loss = new_loss
