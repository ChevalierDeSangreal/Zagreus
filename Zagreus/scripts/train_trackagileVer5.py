...
from Zagreus.models import TrackAgileModuleVer10
from Zagreus.envs import IrisDynamics, task_registry
# os.path.basename(__file__).rstrip(".py")


"""
Based on trackagileVer3
Try to train using vision input
Do not use regression vision input
"""
...
def get_args():
	custom_parameters = [
		...
		{"name": "--experiment_name", "type": str, "default": "track_agileVer5", "help": "Name of the experiment to run or load."},
		...
		# model setting
		{"name": "--param_save_name", "type":str, "default": 'track_agileVer5.pth',
			"help": "The path to model parameters"},
		{"name": "--param_load_path", "type":str, "default": 'track_agileVer3.pth',
			"help": "The path to model parameters"},
...
if __name__ == "__main__":
	...
	model = TrackAgileModuleVer10(device=device).to(device)
	...
	for epoch in range(args.num_epoch):
		...
		input_buffer = torch.zeros(args.slide_size, args.batch_size, 6+64).to(device)
		...
		for step in range(args.len_sample):
			...
			vision_model_output = model.extractor_module(last_dep_image, last_seg_image, dep_image)
			...
			if step % 10 == 0:
				...
				tmp_input = torch.cat((body_vel, now_quad_state[:, 3:6], vision_model_output["feature"]), dim=1)
			...
			if (not (step + 1) % 50):
				...
				loss = loss_agile * 0.9 + loss_pred * 0.1 + loss_vision * 0.1
