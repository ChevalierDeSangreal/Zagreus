...
def get_args():
    ...
    # ------------------- 基本任务参数 -------------------
    parser.add_argument("--experiment_name", type=str, default="test_dynamicsVer0", help="Name of the experiment")
    ...
    parser.add_argument("--param_load_path", type=str, default="worldVere0.pth", help="Path to load pre-trained model")
    ...
    with torch.no_grad():
        for batch_idx, (action_seq, state_seq) in enumerate(loader):
            ...
            for step in range(args.seq_len):
                if step:
                    if step < args.seq_len // 2:
                        now_state, _ = dynamics(action_seq[:, step - 1], state_seq[:, step - 1], dt=0.02)
                    
                    if step == args.seq_len // 2:
                        now_state = state_seq[:, args.seq_len // 2].clone()  # 重置为中间状态
